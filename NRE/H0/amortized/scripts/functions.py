...
def get_Fermat_potentials(x0_lens, y0_lens, ellip, phi, theta_E, gamma_ext, phi_ext, xim_fov, yim_fov):
    ...
    # Parameters
    x0_AGN = 5e-4 * torch.randn((x0_lens.size(0)), device=x0_lens.device)
    y0_AGN = 5e-4 * torch.randn((y0_lens.size(0)), device=y0_lens.device)
...
def param_noise(param, sig=[5e-4, 5e-4, 5e-4, 5e-4, 5e-4, 5e-4, 5e-4]):
    """
...
def kernel_density_estimation(path):
    keys = h5py.File(os.path.join(path, "keys.hdf5"), 'r')
    test_keys = keys["test"][:]
    keys.close()

    # import data
    dataset = h5py.File(os.path.join(path, "dataset.hdf5"), 'r')
    param = dataset["parameters"][test_keys, :7]
    pos = dataset["positions"][test_keys]
    dt = dataset["time_delays"][test_keys]
    dataset.close()

    dt = dt[dt != 0].reshape(-1, 3)

    nim = torch.count_nonzero(torch.from_numpy(dt) + 1, dim=1)
    pot = doub_and_quad_potentials(nim, torch.from_numpy(pos), torch.from_numpy(param))
    pot = pot[pot != -1].reshape(-1, 1)
    pot = pot.detach().cpu().numpy()

    kde = KernelDensity(kernel='gaussian', bandwidth=.05).fit(pot)
...
def gaussian(x, mu, sigma):
    """
        ...
        """
    gauss = np.exp(-.5 * (x - mu) ** 2 / sigma ** 2) / np.sqrt(2 * np.pi * sigma ** 2)
    return gauss


def likelihood(dt, H0, dphi, zd, zs, sig=.35):
    ...
    idx_pad = np.where(dt.flatten() == -1)[0]
    dt_fct = get_time_delays(dphi, zd, Dd, Ds, Dds).flatten()
    dt_fct[idx_pad] = - np.ones_like(idx_pad)
    dt_fct = dt_fct.reshape(-1, 3)

    lkh = gauss(dt_fct, dt, sig).flatten()
    lkh[idx_pad] = np.ones_like(idx_pad)
    lkh = lkh.reshape(-1, 3)

    return lkh


def traditional_posterior(path, dt, H0, dphi, zd, zs):
    nsamp = dt.shape[0]
    npts = H0.shape[0]

    kde = kernel_density_estimation(path)
    prior_pot = kde.score_samples(dphi.reshape(-1, 1))
    idx_pad = np.where(dphi.flatten() == -1)[0]
    prior_pot[idx_pad] = np.ones_like(idx_pad)
    prior_pot = prior_pot.reshape(nsamp, 3)

    lkh_dt = np.zeros(nsamp, npts, 3)
    for i in range(npts):
        lkh_dt[:, i, :] = likelihood(dt, H0[i], dphi, zd, zs)

    posterior = np.prod(lkh_dt * prior_pot, axis=2)
...
def inference(path_data, file_model, path_out, nrow=10, ncol=5, npts=5000, batch_size=200):
    ...
    trad_post = traditional_posterior(path_data, noisy_dt.numpy(), prior, pot.numpy(), z[:, 0], z[:, 1])
    trad_post = normalization(prior_tile, trad_post)
    ...
    for i in range(nrow):
        for j in range(ncol):
            ...
            axes[i, j].plot(prior, trad_post[it], '--', label='{:.2f}'.format(np.max(trad_post[it])))
            min_post = np.minimum(np.min(post[it]), np.min(trad_post[it]))
            max_post = np.maximum(np.max(post[it]), np.max(trad_post[it]))
